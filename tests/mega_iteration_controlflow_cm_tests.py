...
control flow signals (_Return, _Break), and context managers (with_).
...
CM enter/exit lifecycle, nested chains, and exception handler interplay.
"""
...
class ForeachComprehensiveTests(IsolatedAsyncioTestCase):
  ...
  async def test_foreach_sync_fn_on_list(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, [1, 2, 3, 4]).foreach(lambda x: x * 2).run()),
          [2, 4, 6, 8]
        )
  ...
  async def test_foreach_single_element(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, [42]).foreach(lambda x: x + 1).run()),
          [43]
        )
  ...
  async def test_foreach_large_iterable(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, items).foreach(lambda x: x * 3).run()),
          expected
        )
  ...
  async def test_foreach_async_transition_on_middle_element(self):
    ...
    call_count['n'] = 0
    self.assertEqual(
      await Chain([1, 2, 3, 4, 5]).foreach(f).run(),
      [10, 20, 30, 40, 50]
    )
  ...
  async def test_foreach_async_iterable(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, AsyncIterator([1, 2, 3])).foreach(lambda x: x * 5).run()),
          [5, 10, 15]
        )
  ...
  async def test_foreach_custom_iterator(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, CustomIterator([10, 20, 30])).foreach(lambda x: x + 1).run()),
          [11, 21, 31]
        )
    ...
    for fn1, fn2 in product([empty, aempty], repeat=2):
      with self.subTest(fn1=fn1, fn2=fn2):
        def f(el):
          ...
          return fn2(el * 2)
        self.assertEqual(
          await await_(Chain(fn1, [1, 2, 3, 4, 5]).foreach(f).run()),
          [2, 4, 6]
        )
  ...
  async def test_foreach_fn_modifies_mutable_state(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        state.clear()
        self.assertEqual(
          await await_(Chain(fn, [10, 20, 30]).foreach(f).run()),
          [20, 40, 60]
        )
        self.assertEqual(state, [10, 20, 30])

  # -- 19. Foreach with Chain as fn --
  async def test_foreach_chain_as_fn(self):
    """A Chain instance used as the foreach fn argument."""
    inner = Chain().then(lambda x: x * 2).then(lambda x: x + 1)
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, [1, 2, 3]).foreach(inner).run()),
          [3, 5, 7]
        )
  ...
  async def test_foreach_then_chain(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, [1, 2, 3]).foreach(lambda x: x * 2).then(sum).run()),
          12
        )
  ...
  async def test_foreach_async_fn_on_async_iterable(self):
    ...
    self.assertEqual(
      await Chain(AsyncIterator([2, 4, 6])).foreach(lambda x: aempty(x // 2)).run(),
      [1, 2, 3]
    )
...
class FilterComprehensiveTests(IsolatedAsyncioTestCase):
  ...
  async def test_filter_async_predicate(self):
    ...
    self.assertEqual(
      await Chain([1, 2, 3, 4]).filter(lambda x: aempty(x > 2)).run(),
      [3, 4]
    )
  ...
  async def test_filter_async_iterable(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, AsyncIterator([10, 20, 30, 40])).filter(lambda x: x >= 30).run()),
          [30, 40]
        )
  ...
  async def test_filter_falsy_predicate_results_filter_out(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, [1, 2, 3]).filter(lambda x: 0).run()),
          []
        )
        self.assertEqual(
          await await_(Chain(fn, [1, 2, 3]).filter(lambda x: '').run()),
          []
        )
        self.assertEqual(
  ...
  async def test_filter_truthy_predicate_keeps_element(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, ['a', 'b', 'c']).filter(lambda x: 1).run()),
          ['a', 'b', 'c']
        )
        self.assertEqual(
          await await_(Chain(fn, ['a', 'b', 'c']).filter(lambda x: 'yes').run()),
          ['a', 'b', 'c']
        )
        self.assertEqual(
          await await_(Chain(fn, ['a', 'b', 'c']).filter(lambda x: [1]).run()),
  ...
  async def test_filter_then_foreach(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, [1, 2, 3, 4, 5, 6])
  ...
  async def test_filter_then_chain(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, [1, 2, 3, 4, 5]).filter(lambda x: x > 3).then(len).run()),
          2
        )
  ...
  async def test_filter_complex_predicate(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, list(range(20))).filter(
  ...
  async def test_filter_single_element_passes(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, [42]).filter(lambda x: x > 0).run()),
          [42]
        )
...
class GatherComprehensiveTests(IsolatedAsyncioTestCase):
  ...
  async def test_gather_identical_functions(self):
    ...
    fn = lambda v: v * 2
    for fn_ in [empty, aempty]:
      with self.subTest(fn_=fn_):
        self.assertEqual(
          await await_(Chain(fn_, 7).gather(fn, fn, fn).run()),
          [14, 14, 14]
        )
...
class ContextManagerComprehensiveTests(IsolatedAsyncioTestCase):
  ...
  async def test_with_exit_raises(self):
    ...
    try:
      ...
      result_exc = e
    self.assertIs(result_exc, exit_exc)
  ...
  async def test_with_async_cm_basic(self):
    ...
    self.assertEqual(result, 'got_async_hello')
    self.assertTrue(cm.entered)
    self.assertTrue(cm.exited)
  ...
  async def test_with_async_cm_async_body(self):
    ...
    self.assertEqual(result, 'async_actx')
...
class ReturnControlFlowTests(IsolatedAsyncioTestCase):
  ...
  async def test_return_in_top_level_chain(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, 10).then(lambda v: Chain.return_(v * 2)).run()),
          20
        )
  ...
  async def test_return_in_nested_chain(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        ...
        self.assertEqual(
          await await_(Chain(fn, 5).then(inner).then(lambda v: v + 1).run()),
          50
        )
  ...
  async def test_return_with_literal_value(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, 1).then(lambda v: Chain.return_(42)).run()),
          42
        )
  ...
  async def test_return_with_callable_value(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, 1).then(lambda v: Chain.return_(lambda: 42)).run()),
          42
        )
  ...
  async def test_return_with_callable_and_args(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, 1).then(lambda v: Chain.return_(int, '42')).run()),
          42
        )
  ...
  async def test_return_with_null_value(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertIsNone(
          await await_(Chain(fn, 99).then(lambda v: Chain.return_()).run())
        )
  ...
  async def test_return_with_async_callable(self):
    ...
    self.assertEqual(
      await Chain(1).then(lambda v: Chain.return_(aempty, 42)).run(),
      42
    )
  ...
  async def test_return_deeply_nested(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        ...
        self.assertEqual(
          await await_(Chain(fn, 2).then(inner1).then(lambda v: v + 1).run()),
          2000
        )
  ...
  async def test_return_in_simple_chain(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        self.assertEqual(
          await await_(Chain(fn, 7).then(lambda v: Chain.return_(v + 3)).run()),
          10
        )

  # -- 81. _Return with multiple links after it --
  async def test_return_skips_subsequent_links(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        executed.clear()
        self.assertEqual(
          await await_(Chain(fn, 1).then(lambda v: Chain.return_(42)).then(track).run()),
          42
        )
...
class BreakControlFlowTests(IsolatedAsyncioTestCase):
  ...
  async def test_break_in_nested_chain_propagates(self):
    ...
    for fn1, fn2 in product([empty, aempty], repeat=2):
      with self.subTest(fn1=fn1, fn2=fn2):
        ...
        self.assertEqual(
          await await_(Chain(fn1, [1, 2, 3, 4, 5]).then(inner).run()),
          [10, 20]
        )
  ...
  async def test_break_in_async_foreach(self):
    ...
    def f(el):
      ...
      return el * 2
    self.assertEqual(
      await Chain(AsyncIterator([1, 2, 3, 4, 5])).foreach(f).run(),
      [2, 4]
    )
...
class GeneratorIterateComprehensiveTests(IsolatedAsyncioTestCase):
...
class EdgeCasesInteractionsTests(IsolatedAsyncioTestCase):
  ...
  async def test_foreach_with_finally_handler(self):
    ...
    for fn in [empty, aempty]:
      with self.subTest(fn=fn):
        ...
        self.assertTrue(finally_called['called'])
  ...
  async def test_break_in_async_chain_foreach(self):
    ...
    def f(el):
      ...
      return el * 10
    self.assertEqual(
      await Chain(aempty, [1, 2, 3, 4]).foreach(f).run(),
      [10, 20]
    )

  # -- 112. Multiple breaks in different foreach calls --
    ...
    for fn1, fn2 in product([empty, aempty], repeat=2):
      with self.subTest(fn1=fn1, fn2=fn2):
        ...
        result1 = await await_(
          Chain(fn1, [1, 2, 3, 4]).foreach(f1).run()
        )
        ...
        result2 = await await_(
          Chain(fn1, [1, 2, 3]).foreach(f2).run()
        )
...
class AsyncSpecificEdgeCaseTests(IsolatedAsyncioTestCase):
  ...
  async def test_foreach_all_elements_async(self):
    ...
    self.assertEqual(
      await Chain([1, 2, 3]).foreach(lambda x: aempty(x + 100)).run(),
      [101, 102, 103]
    )
  ...
  async def test_with_async_cm_async_body_async_chain(self):
    ...
    self.assertEqual(result, 'body_fully_async')
  ...
  async def test_gather_all_coroutines_use_asyncio_gather(self):
    ...
    self.assertEqual(sorted(results), ['f1', 'f2'])
  ...
  async def test_async_cm_exit_ordering_on_exception(self):
    ...
    with self.assertRaises(TestExc):
      await Chain(cm).with_(body_raises).run()
    self.assertTrue(cm.exited)
    self.assertEqual(cm.exit_args[0], TestExc)
    self.assertIs(cm.exit_args[1], exc)
