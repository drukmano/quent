...
# ---------------------------------------------------------------------------
class GeneratorCallNestingTests(IsolatedAsyncioTestCase):
...
# ---------------------------------------------------------------------------
class IterateAdvancedTests(IsolatedAsyncioTestCase):
  ...
  async def test_iterate_over_range(self):
    ...
    for i in Chain(range, 5).iterate(lambda i: i + 10):
      r.append(i)
    self.assertEqual(r, [10, 11, 12, 13, 14])
  ...
  async def test_iterate_over_set(self):
    ...
    for i in Chain(lambda: {10, 20, 30}).iterate():
      r.append(i)
    self.assertEqual(sorted(r), [10, 20, 30])
