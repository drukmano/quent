...
"""Thread safety tests for concurrent execution of shared Quent pipeline instances.

Tests validate SPEC §3 guarantee: a fully constructed pipeline is safe to execute
...
# ---------------------------------------------------------------------------
# Helper: run pipeline from N threads, collect results
# ---------------------------------------------------------------------------


def _run_concurrent(q: Q, n: int, run_value=None) -> list:
...
def _run_concurrent_varying(q: Q, values: list) -> list:
...
# ---------------------------------------------------------------------------
# §3: Basic concurrent run — then-pipeline
# ---------------------------------------------------------------------------
...
class BasicConcurrentRunTest(TestCase):
  ...
  def test_simple_then_chain_concurrent(self):
    """Shared then-pipeline produces same result from all threads simultaneously."""
    q = Q(10).then(lambda x: x + 1).then(lambda x: x * 2)
    results = _run_concurrent(q, THREAD_COUNT)
  ...
  def test_multi_step_then_chain_concurrent(self):
    ...
    q = Q(1).then(lambda x: x + 1).then(lambda x: x * 3).then(lambda x: x - 2).then(lambda x: x + 10)
    # 1 -> 2 -> 6 -> 4 -> 14
    results = _run_concurrent(q, THREAD_COUNT)
  ...
  def test_run_value_shared_chain(self):
    ...
    results = _run_concurrent(q, THREAD_COUNT, run_value=5)
    # 5 -> 10 -> 11
    self.assertEqual(results, [11] * THREAD_COUNT)


# ---------------------------------------------------------------------------
# §3: Various pipeline shapes — concurrent correctness
# ---------------------------------------------------------------------------
...
class ChainShapeConcurrentTest(TestCase):
  ...
  def test_foreach_concurrent_outer(self):
    """Pipeline with foreach() is safe to run from multiple threads concurrently."""
    q = Q([1, 2, 3, 4]).foreach(lambda x: x * 2)
    results = _run_concurrent(q, THREAD_COUNT)
  ...
  def test_foreach_with_inner_concurrency(self):
    ...
    q = Q([1, 2, 3, 4]).foreach(lambda x: x + 10, concurrency=4)
    results = _run_concurrent(q, THREAD_COUNT)
    self.assertEqual(results, [[11, 12, 13, 14]] * THREAD_COUNT)

  def test_foreach_do_concurrent_outer(self):
    """Pipeline with foreach_do() is safe to run from multiple threads concurrently."""
    q = Q([10, 20, 30]).foreach_do(lambda x: x + 1)
  ...
  def test_foreach_do_with_inner_concurrency(self):
    ...
    q = Q([1, 2, 3]).foreach_do(lambda x: x * 100, concurrency=3)
    results = _run_concurrent(q, THREAD_COUNT)
    self.assertEqual(results, [[1, 2, 3]] * THREAD_COUNT)

  def test_gather_concurrent_outer(self):
    """Pipeline with gather() is safe to run from multiple threads concurrently."""
    q = Q(5).gather(lambda x: x + 1, lambda x: x * 2, lambda x: x - 1)
    results = _run_concurrent(q, THREAD_COUNT)
    self.assertEqual(results, [(6, 10, 4)] * THREAD_COUNT)

  def test_nested_chain_concurrent(self):
    """Pipeline with a nested Q pipeline step is safe under concurrent execution."""
    inner = Q().then(lambda x: x + 100)
    outer = Q(7).then(inner)
  ...
  def test_except_handler_concurrent(self):
    """Pipeline with except_() handler executes correctly from multiple threads."""
    q = Q().then(lambda x: 1 / x).except_(lambda exc: -1)
    results = _run_concurrent(q, THREAD_COUNT, run_value=0)
  ...
  def test_except_handler_no_error_concurrent(self):
    """Pipeline with except_() that never fires is safe under concurrent execution."""
    q = Q().then(lambda x: x * 3).except_(lambda exc: -999)
    results = _run_concurrent(q, THREAD_COUNT, run_value=4)
    self.assertEqual(results, [12] * THREAD_COUNT)

  def test_finally_handler_concurrent(self):
    """Pipeline with finally_() handler executes correctly from multiple threads."""
    counter = [0]
    lock = threading.Lock()
    ...
    q = Q().then(lambda x: x + 1).finally_(_cleanup)
    results = _run_concurrent(q, THREAD_COUNT, run_value=10)
    self.assertEqual(results, [11] * THREAD_COUNT)
    self.assertEqual(counter[0], THREAD_COUNT)

  def test_if_conditional_concurrent(self):
    """Pipeline with if_().then() conditional is safe under concurrent execution."""
    q = Q().if_(lambda x: x > 0).then(lambda x: x * 10).else_(lambda x: -x)
    results = _run_concurrent(q, THREAD_COUNT, run_value=5)
    self.assertEqual(results, [50] * THREAD_COUNT)

  def test_if_else_branch_concurrent(self):
    """Pipeline with if_().then().else_() else branch is safe under concurrent execution."""
    q = Q().if_(lambda x: x > 0).then(lambda x: x * 10).else_(lambda x: -x)
    results = _run_concurrent(q, THREAD_COUNT, run_value=-3)
    self.assertEqual(results, [3] * THREAD_COUNT)

  def test_with_context_manager_concurrent(self):
    """Pipeline with with_() context manager step is safe under concurrent execution."""

    @contextlib.contextmanager
    def _make_ctx(value):
      yield value * 2

    q = Q().then(_make_ctx).with_(lambda ctx: ctx + 1)
    results = _run_concurrent(q, THREAD_COUNT, run_value=5)
    # value=5 -> ctx=10 -> ctx+1=11
    self.assertEqual(results, [11] * THREAD_COUNT)

  def test_do_side_effect_concurrent(self):
    """Pipeline with do() side-effect step is safe under concurrent execution."""
    side_effects = []
    lock = threading.Lock()
    ...
    q = Q().do(_record).then(lambda x: x + 1)
    results = _run_concurrent(q, THREAD_COUNT, run_value=7)
...
class PerThreadValueIsolationTest(TestCase):
  ...
  def test_different_inputs_produce_different_outputs(self):
    ...
    q = Q().then(lambda x: x * x)
    values = list(range(THREAD_COUNT))
    results = _run_concurrent_varying(q, values)
    expected = [v * v for v in values]
    self.assertEqual(results, expected)

  def test_multi_step_per_thread_isolation(self):
    """Multi-step pipeline with per-thread input produces isolated correct results."""
    q = Q().then(lambda x: x + 1).then(lambda x: x * 2).then(str)
    values = list(range(THREAD_COUNT))
    results = _run_concurrent_varying(q, values)
  ...
  def test_gather_per_thread_isolation(self):
    ...
    q = Q().gather(lambda x: x + 1, lambda x: x * 2)
    values = list(range(THREAD_COUNT))
    results = _run_concurrent_varying(q, values)
  ...
  def test_conditional_per_thread_isolation(self):
    ...
    q = Q().if_(lambda x: x % 2 == 0).then(lambda x: x * 10).else_(lambda x: x * 100)
    values = list(range(THREAD_COUNT))
    results = _run_concurrent_varying(q, values)
  ...
  def test_except_per_thread_isolation(self):
    ...
    # Pipeline that divides by input: 0 -> error -> handler, nonzero -> result
    q = Q().then(lambda x: 100 // x).except_(lambda exc: -1)
...
class OnStepConcurrentTest(TestCase):
  ...
  def test_on_step_fires_for_all_threads(self):
    ...
    q = Q(5).then(lambda x: x + 1)
    # Each pipeline.run() fires 2 callbacks: 'root' and 'then'
    _run_concurrent(q, THREAD_COUNT)
    with self._lock:
      total = len(self._log)
  ...
  def test_on_step_thread_safe_collection(self):
    ...
    q = Q(1).then(lambda x: x + 1).then(lambda x: x * 3)
...
class CloneIndependenceConcurrentTest(TestCase):
  ...
  def test_original_and_clone_run_concurrently(self):
    ...
    original = Q(100).then(lambda x: x + 1)
  ...
  def test_multiple_clones_concurrent(self):
    ...
    original = Q(5).then(lambda x: x + 1)
  ...
  def test_clone_nested_chain_independence(self):
    """Cloned chains with nested Q pipeline steps maintain independent execution."""
    inner = Q().then(lambda x: x + 50)
    original = Q(1).then(inner)
    clone = original.clone()
...
class DecoratorConcurrentTest(TestCase):
  ...
  def test_decorator_concurrent_calls(self):
    ...
    process = Q().then(lambda x: x * 3).then(lambda x: x + 1).as_decorator()
  ...
  def test_decorator_same_instance_concurrent(self):
    ...
    add_ten = Q().then(lambda x: x + 10).as_decorator()
  ...
  def test_decorator_error_handling_concurrent(self):
    ...
    handle_div = Q().then(lambda x: 100 // x).except_(lambda exc: -1).as_decorator()
...
class StressConcurrentTest(TestCase):
  ...
  def test_stress_simple_chain(self):
    ...
    q = Q().then(lambda x: x + 1).then(lambda x: x * 2)
    errors = []
    lock = threading.Lock()
  ...
  def test_stress_gather_chain(self):
    ...
    q = Q().gather(lambda x: x + 1, lambda x: x - 1)
    errors = []
    lock = threading.Lock()
  ...
  def test_stress_except_chain(self):
    ...
    q = Q().then(_safe_div).except_(lambda exc: -1)
    errors = []
    lock = threading.Lock()
  ...
  def test_stress_nested_chain(self):
    """STRESS: nested Q pipeline steps under 50 threads x 100 iterations produce no errors."""
    inner = Q().then(lambda x: x + 10)
    outer = Q().then(inner).then(lambda x: x * 2)
    errors = []
    lock = threading.Lock()
  ...
  def test_stress_conditional_chain(self):
    ...
    q = Q().if_(lambda x: x > 0).then(lambda x: x * 2).else_(lambda x: 0)
    errors = []
    lock = threading.Lock()
