...
class TestIfSyncPredicateAsyncFn(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_truthy_predicate_async_fn(self):
    r = await Chain(10).if_(lambda v: v > 5, async_double).run()
    self.assertEqual(r, 20)

  async def test_falsy_predicate_async_fn_passthrough(self):
    r = Chain(3).if_(lambda v: v > 5, async_double).run()
    self.assertEqual(r, 3)

  async def test_truthy_predicate_async_fn_returns_none(self):
    r = await Chain(10).if_(lambda v: v > 5, async_return_none).run()
    self.assertIsNone(r)
...
class TestIfAsyncPredicateAsyncFn(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_async_truthy_predicate_async_fn(self):
    r = await Chain(10).if_(async_truthy_pred, async_double).run()
    self.assertEqual(r, 20)

  async def test_async_falsy_predicate_async_fn_passthrough(self):
    r = await Chain(3).if_(async_truthy_pred, async_double).run()
    self.assertEqual(r, 3)

  async def test_async_boundary_predicate_async_fn(self):
    r = await Chain(0).if_(async_boundary_pred, async_double).run()
    self.assertEqual(r, 0)  # 0 * 2 = 0

  async def test_async_boundary_predicate_nonzero_passthrough(self):
    r = await Chain(5).if_(async_boundary_pred, async_double).run()
    self.assertEqual(r, 5)

  async def test_async_truthy_predicate_async_ellipsis_fn(self):
    r = await Chain(10).if_(async_truthy_pred, async_no_arg_fn, ...).run()
...
class TestIfElseSyncSync(unittest.TestCase):
  ...
  def test_truthy_takes_if_branch_not_else(self):
    r = Chain(10).if_(lambda v: v > 5, lambda v: v * 2).else_(lambda v: v * 3).run()
    self.assertEqual(r, 20)

  def test_falsy_takes_else_branch(self):
    r = Chain(3).if_(lambda v: v > 5, lambda v: v * 2).else_(lambda v: v * 3).run()
    self.assertEqual(r, 9)

  def test_falsy_else_plain_value(self):
    r = Chain(3).if_(lambda v: v > 5, lambda v: v * 2).else_('fallback').run()
    self.assertEqual(r, 'fallback')

  def test_truthy_ignores_else_plain_value(self):
    r = Chain(10).if_(lambda v: v > 5, lambda v: v * 2).else_('fallback').run()
    self.assertEqual(r, 20)

  def test_falsy_else_with_extra_args(self):
    r = Chain(3).if_(lambda v: v > 5, lambda v: v * 2).else_(lambda a, b: a - b, 100, 50).run()
...
class TestIfElseAsyncVariants(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_falsy_async_else_fn(self):
    r = await Chain(3).if_(lambda v: v > 5, lambda v: v * 2).else_(async_else_fn).run()
    self.assertEqual(r, 300)

  async def test_truthy_ignores_async_else_fn(self):
    r = Chain(10).if_(lambda v: v > 5, lambda v: v * 2).else_(async_else_fn).run()
    self.assertEqual(r, 20)

  async def test_async_pred_falsy_sync_else(self):
    r = await Chain(3).if_(async_truthy_pred, lambda v: v * 2).else_(lambda v: v + 1000).run()
    self.assertEqual(r, 1003)

  async def test_async_pred_falsy_async_else(self):
    r = await Chain(3).if_(async_truthy_pred, async_double).else_(async_else_fn).run()
    self.assertEqual(r, 300)

  async def test_async_pred_truthy_ignores_else(self):
    r = await Chain(10).if_(async_truthy_pred, async_double).else_(async_else_fn).run()
    self.assertEqual(r, 20)

  async def test_async_pred_falsy_else_plain_value(self):
    r = await Chain(3).if_(async_truthy_pred, async_double).else_('default').run()
...
class TestIfCurrentValueStates(unittest.TestCase):
  ...
  def test_truthy_value(self):
    r = Chain(10).if_(lambda v: v > 0, lambda v: v + 1).run()
    self.assertEqual(r, 11)

  def test_falsy_value_zero(self):
    r = Chain(0).if_(lambda v: v, lambda v: 'should_not').run()
    self.assertEqual(r, 0)

  def test_falsy_value_empty_string(self):
    r = Chain('').if_(lambda v: v, lambda v: 'should_not').run()
    self.assertEqual(r, '')
  ...
  def test_none_as_current_value_passthrough(self):
    r = Chain(None).if_(lambda v: v is not None, lambda v: v + 1).run()
    self.assertIsNone(r)

  def test_none_as_current_value_truthy_pred(self):
    r = Chain(None).if_(lambda v: v is None, lambda v: 'was_none').run()
  ...
  def test_no_root_value_run_provides_value_truthy(self):
    ...
    r = Chain().if_(lambda v: v > 5, lambda v: v * 2).run(10)
    self.assertEqual(r, 20)

  def test_no_root_value_run_provides_value_falsy(self):
    r = Chain().if_(lambda v: v > 5, lambda v: v * 2).run(3)
    self.assertEqual(r, 3)

  def test_no_root_value_run_provides_value_with_else(self):
    r = Chain().if_(lambda v: v > 5, lambda v: v * 2).else_(lambda v: v * 3).run(3)
    self.assertEqual(r, 9)

  def test_falsy_value_zero_with_else(self):
    r = Chain(0).if_(lambda v: v, lambda v: 'truthy').else_(lambda v: 'falsy').run()
    self.assertEqual(r, 'falsy')

  def test_none_value_with_else(self):
    r = Chain(None).if_(lambda v: v is not None, lambda v: 'not_none').else_(lambda v: 'is_none').run()
...
class TestIfChainContext(unittest.TestCase):
  ...
  def test_if_after_then_steps(self):
    r = Chain(2).then(lambda v: v + 3).then(lambda v: v * 2).if_(lambda v: v > 5, lambda v: v + 100).run()
    # 2 -> 5 -> 10 -> 10 > 5 is True -> 110
    self.assertEqual(r, 110)

  def test_if_before_then_steps_truthy(self):
    r = Chain(10).if_(lambda v: v > 5, lambda v: v * 2).then(lambda v: v + 1).run()
    # 10 -> 20 -> 21
    self.assertEqual(r, 21)

  def test_if_before_then_steps_falsy_passthrough(self):
    r = Chain(3).if_(lambda v: v > 5, lambda v: v * 2).then(lambda v: v + 1).run()
    # 3 passthrough -> 4
  ...
  def test_multiple_if_in_sequence(self):
    r = (
      Chain(10)
      .if_(lambda v: v > 5, lambda v: v * 2)      # 10 > 5 -> 20
      .if_(lambda v: v > 15, lambda v: v + 100)    # 20 > 15 -> 120
      .run()
    )
  ...
  def test_multiple_if_some_truthy_some_falsy(self):
    r = (
      Chain(10)
      .if_(lambda v: v > 5, lambda v: v * 2)       # 10 > 5 -> 20
      .if_(lambda v: v > 50, lambda v: v + 1000)   # 20 > 50 -> passthrough 20
      .run()
    )
    self.assertEqual(r, 20)
  ...
  def test_multiple_if_else_in_sequence(self):
    r = (
      Chain(3)
      .if_(lambda v: v > 5, lambda v: 'big').else_(lambda v: 'small')    # 'small'
      .if_(lambda v: v == 'small', lambda v: 'confirmed').else_(lambda v: 'unexpected')
      .run()
    )
...
class TestIfFrozenChain(unittest.TestCase):
  ...
  def test_if_in_frozen_chain_truthy(self):
    fc = Chain().if_(lambda v: v > 5, lambda v: v * 2).freeze()
    r = fc(10)
    self.assertEqual(r, 20)
  ...
  def test_if_else_in_frozen_chain_truthy(self):
    fc = Chain().if_(lambda v: v > 5, lambda v: v * 2).else_(lambda v: v * 3).freeze()
    r = fc(10)
    self.assertEqual(r, 20)
  ...
  def test_frozen_chain_used_as_nested_fn(self):
    fc = Chain().then(lambda v: v + 1).if_(lambda v: v > 5, lambda v: v * 10).freeze()
...
class TestIfFrozenChainAsync(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_frozen_if_async_predicate(self):
    fc = Chain().if_(async_truthy_pred, lambda v: v * 2).freeze()
    r = await fc(10)
    self.assertEqual(r, 20)

  async def test_frozen_if_async_fn(self):
    fc = Chain().if_(lambda v: v > 5, async_double).freeze()
    r = await fc(10)
    self.assertEqual(r, 20)
...
class TestIfElseErrors(unittest.TestCase):
  ...
  def test_else_after_then_after_if_raises(self):
    with self.assertRaises(QuentException) as ctx:
      Chain(10).if_(lambda v: True, lambda v: v).then(lambda v: v).else_(lambda v: v)
    self.assertIn('else_() can only be used immediately after if_()', str(ctx.exception))
  ...
  def test_predicate_raises_propagates(self):
    ...
    with self.assertRaises(ValueError) as ctx:
      Chain(10).if_(bad_pred, lambda v: v * 2).run()
  ...
  def test_fn_raises_propagates(self):
    ...
    with self.assertRaises(RuntimeError) as ctx:
      Chain(10).if_(lambda v: True, bad_fn).run()
  ...
  def test_else_fn_raises_propagates(self):
    ...
    with self.assertRaises(TypeError) as ctx:
      Chain(3).if_(lambda v: v > 5, lambda v: v * 2).else_(bad_else).run()
...
class TestIfElseErrorsAsync(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_async_predicate_raises(self):
    with self.assertRaises(RuntimeError) as ctx:
      await Chain(10).if_(async_pred_raises, lambda v: v * 2).run()
    self.assertIn('async predicate error', str(ctx.exception))

  async def test_async_fn_raises(self):
    with self.assertRaises(RuntimeError) as ctx:
      await Chain(10).if_(lambda v: True, async_raise).run()
  ...
  async def test_async_else_fn_raises(self):
    ...
    with self.assertRaises(TypeError) as ctx:
      await Chain(3).if_(lambda v: v > 5, lambda v: v).else_(bad_async_else).run()
    self.assertIn('async else boom', str(ctx.exception))

  async def test_async_pred_raises_with_else(self):
    with self.assertRaises(RuntimeError) as ctx:
      await Chain(10).if_(async_pred_raises, lambda v: v).else_(lambda v: 'else').run()
    self.assertIn('async predicate error', str(ctx.exception))
  ...
  async def test_fn_raises_after_async_truthy_pred(self):
    ...
    with self.assertRaises(RuntimeError) as ctx:
      await Chain(10).if_(async_truthy_pred, bad_fn).run()
...
class TestIfWithExceptFinally(unittest.TestCase):
  ...
  def test_if_predicate_raises_caught_by_except(self):
    ...
    r = (
      Chain(10)
      .if_(bad_pred, lambda v: v)
      .except_(lambda exc: 'caught_pred')
      .run()
    )
  ...
  def test_else_fn_raises_caught_by_except(self):
    ...
    r = (
      Chain(3)
      .if_(lambda v: v > 5, lambda v: v)
      .else_(bad_else)
      .except_(lambda exc: 'caught_else')
      .run()
    )
...
class TestIfNestedChainFn(unittest.TestCase):
  ...
  def test_nested_chain_receives_current_value(self):
    ...
    r = Chain(7).if_(lambda v: v > 5, inner).run()
  ...
  def test_nested_chain_in_else(self):
    ...
    r = Chain(3).if_(lambda v: v > 5, inner_if).else_(inner_else).run()
...
class TestIfNestedChainAsync(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_nested_chain_with_async_step(self):
    ...
    r = await Chain(10).if_(lambda v: v > 5, inner).run()
    self.assertEqual(r, 20)
...
class TestIfWithRunValue(unittest.TestCase):
  ...
  def test_if_on_valueless_chain_run_provides_value_truthy(self):
    c = Chain().if_(lambda v: v > 5, lambda v: v * 2)
    r = c.run(10)
    self.assertEqual(r, 20)
...
class TestIfComplexCombinations(unittest.TestCase):
  ...
  def test_if_then_if_chained(self):
    r = (
      Chain(10)
      .if_(lambda v: v > 5, lambda v: v * 2)       # 20
      .then(lambda v: v + 1)                        # 21
      .if_(lambda v: v > 20, lambda v: v * 10)      # 210
      .run()
    )
  ...
  def test_if_else_then_if_else_chained(self):
    r = (
      Chain(3)
      .if_(lambda v: v > 5, lambda v: 'big').else_(lambda v: 'small')
      .then(lambda v: v.upper())
      .if_(lambda v: v == 'SMALL', lambda v: 'confirmed_small').else_(lambda v: 'unexpected')
      .run()
    )
  ...
  def test_multiple_if_all_falsy_passthrough(self):
    r = (
      Chain(1)
      .if_(lambda v: v > 10, lambda v: 'a')
      .if_(lambda v: v > 20, lambda v: 'b')
      .if_(lambda v: v > 30, lambda v: 'c')
      .run()
    )
  ...
  def test_multiple_if_all_truthy_last_wins(self):
    r = (
      Chain(100)
      .if_(lambda v: v > 1, lambda v: v + 1)    # 101
      .if_(lambda v: v > 1, lambda v: v + 2)    # 103
      .if_(lambda v: v > 1, lambda v: v + 3)    # 106
      .run()
    )
    self.assertEqual(r, 106)

  def test_if_with_string_value(self):
    r = Chain('hello').if_(lambda v: len(v) > 3, lambda v: v.upper()).run()
    self.assertEqual(r, 'HELLO')

  def test_if_with_list_value(self):
    r = Chain([1, 2, 3]).if_(lambda v: len(v) > 2, lambda v: sum(v)).run()
    self.assertEqual(r, 6)

  def test_if_with_dict_value(self):
    r = Chain({'a': 1}).if_(lambda v: 'a' in v, lambda v: v['a']).run()
    self.assertEqual(r, 1)
...
class TestIfComplexAsync(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_then_async_then_if_sync(self):
    r = await Chain(5).then(async_add_one).if_(lambda v: v > 5, lambda v: v * 10).run()
    # 5 -> async 6 -> 6 > 5 -> 60
    self.assertEqual(r, 60)

  async def test_if_async_fn_then_sync_then(self):
    r = await Chain(10).if_(lambda v: v > 5, async_double).then(lambda v: v + 1).run()
    # 10 > 5 -> async 20 -> 21
    self.assertEqual(r, 21)

  async def test_if_falsy_async_fn_then_sync_then(self):
    r = await Chain(3).if_(lambda v: v > 5, async_double).then(async_add_one).run()
    # 3 passthrough -> async 4
    self.assertEqual(r, 4)

  async def test_chain_with_both_async_pred_and_async_fn_and_else(self):
    r = await Chain(10).if_(async_truthy_pred, async_double).else_(async_triple).run()
    self.assertEqual(r, 20)

    r2 = await Chain(3).if_(async_truthy_pred, async_double).else_(async_triple).run()
    self.assertEqual(r2, 9)

  async def test_multiple_if_async_in_sequence(self):
    r = await (
      Chain(10)
      .if_(async_truthy_pred, async_double)            # 10 > 5 -> 20
      .if_(lambda v: v > 15, async_add_one)            # 20 > 15 -> 21
      .run()
    )
    self.assertEqual(r, 21)
...
class TestIfEdgeCases(unittest.TestCase):
  ...
  def test_predicate_returns_truthy_nonbool(self):
    ...
    r = Chain(10).if_(lambda v: 1, lambda v: 'yes').run()
    self.assertEqual(r, 'yes')

  def test_predicate_returns_falsy_nonbool_zero(self):
    r = Chain(10).if_(lambda v: 0, lambda v: 'yes').run()
    self.assertEqual(r, 10)

  def test_predicate_returns_falsy_nonbool_none(self):
    r = Chain(10).if_(lambda v: None, lambda v: 'yes').run()
    self.assertEqual(r, 10)

  def test_predicate_returns_falsy_nonbool_empty_string(self):
    r = Chain(10).if_(lambda v: '', lambda v: 'yes').run()
    self.assertEqual(r, 10)

  def test_predicate_returns_truthy_nonbool_string(self):
    r = Chain(10).if_(lambda v: 'nonempty', lambda v: 'yes').run()
    self.assertEqual(r, 'yes')
  ...
  def test_fn_replaces_value_with_false(self):
    r = Chain(10).if_(lambda v: True, lambda v: False).run()
    self.assertIs(r, False)

  def test_fn_replaces_value_with_zero(self):
    r = Chain(10).if_(lambda v: True, lambda v: 0).run()
    self.assertEqual(r, 0)

  def test_fn_replaces_value_with_empty_string(self):
    r = Chain(10).if_(lambda v: True, lambda v: '').run()
    self.assertEqual(r, '')

  def test_fn_plain_value_none(self):
    r = Chain(10).if_(lambda v: True, None).run()
    self.assertIsNone(r)

  def test_fn_plain_value_zero(self):
    r = Chain(10).if_(lambda v: True, 0).run()
    self.assertEqual(r, 0)

  def test_fn_plain_value_false(self):
    r = Chain(10).if_(lambda v: True, False).run()
    self.assertIs(r, False)

  def test_else_fn_returns_none(self):
    r = Chain(3).if_(lambda v: v > 5, lambda v: v).else_(lambda v: None).run()
    self.assertIsNone(r)

  def test_else_plain_value_none(self):
    r = Chain(3).if_(lambda v: v > 5, lambda v: v).else_(None).run()
    self.assertIsNone(r)

  def test_else_plain_value_zero(self):
    r = Chain(3).if_(lambda v: v > 5, lambda v: v).else_(0).run()
    self.assertEqual(r, 0)

  def test_else_plain_value_false(self):
    r = Chain(3).if_(lambda v: v > 5, lambda v: v).else_(False).run()
    self.assertIs(r, False)

  def test_if_true_preserves_type(self):
    r = Chain(10).if_(lambda v: True, lambda v: v).run()
    self.assertIsInstance(r, int)
    self.assertEqual(r, 10)

  def test_if_false_preserves_type(self):
    r = Chain(10).if_(lambda v: False, lambda v: 'changed').run()
    self.assertIsInstance(r, int)
    self.assertEqual(r, 10)
...
class TestIfWithCallAlias(unittest.TestCase):
  ...
  def test_chain_call_truthy(self):
    r = Chain(10).if_(lambda v: v > 5, lambda v: v * 2)()
    self.assertEqual(r, 20)
...
class TestIfNullPassthrough(unittest.TestCase):
  ...
  def test_no_root_run_value_falsy_pred_returns_passthrough(self):
    ...
    r = Chain().if_(lambda v: v > 100, lambda v: 'yes').run(5)
    self.assertEqual(r, 5)

  def test_no_root_run_value_truthy_pred_returns_fn_result(self):
    r = Chain().if_(lambda v: v > 0, lambda v: 'yes').run(5)
    self.assertEqual(r, 'yes')

  def test_no_root_run_value_if_else_takes_if(self):
    r = Chain().if_(lambda v: v > 0, lambda v: 'if_val').else_(lambda v: 'else_val').run(5)
    self.assertEqual(r, 'if_val')

  def test_no_root_run_value_if_else_takes_else(self):
    r = Chain().if_(lambda v: v > 100, lambda v: 'if_val').else_(lambda v: 'else_val').run(5)
...
class TestIfMixedSyncAsyncPaths(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_sync_pred_falsy_async_else(self):
    r = await Chain(3).if_(lambda v: v > 5, lambda v: v * 2).else_(async_triple).run()
    self.assertEqual(r, 9)

  async def test_async_pred_truthy_sync_fn_sync_else(self):
    r = await Chain(10).if_(async_truthy_pred, lambda v: v * 2).else_(lambda v: v * 3).run()
    self.assertEqual(r, 20)

  async def test_async_pred_falsy_sync_fn_sync_else(self):
    r = await Chain(3).if_(async_truthy_pred, lambda v: v * 2).else_(lambda v: v * 3).run()
    self.assertEqual(r, 9)
  ...
  async def test_async_pred_falsy_async_fn_async_else(self):
    r = await Chain(3).if_(async_truthy_pred, async_double).else_(async_triple).run()
    self.assertEqual(r, 9)
...
class TestIfWithControlFlow(unittest.TestCase):
  ...
  def test_return_inside_if_fn(self):
    r = (
      Chain(10)
      .if_(lambda v: v > 5, lambda v: Chain.return_(999))
...
class TestIfWithControlFlowAsync(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_async_return_inside_if_fn(self):
    r = await (
      Chain(10)
      .if_(async_truthy_pred, lambda v: Chain.return_(999))
      .then(lambda v: 'should_not_reach')
      .run()
    )
    self.assertEqual(r, 999)
  ...
  async def test_async_return_inside_else_fn(self):
    r = await (
      Chain(3)
      .if_(async_truthy_pred, lambda v: v)
      .else_(lambda v: Chain.return_(888))
      .then(lambda v: 'should_not_reach')
      .run()
    )
