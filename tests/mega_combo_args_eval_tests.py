...
class TestDetermineEvalCodeMatrix(MyTestCase):
  ...
  async def test_callable_no_args_with_kwargs(self):
    ...
    await self.assertEqual(result, {'x': 1, 'y': 2})
  ...
  async def test_callable_with_args_and_kwargs(self):
    ...
    await self.assertEqual(result, (5, 20))
  ...
  async def test_ellipsis_triggers_call_without_args(self):
    ...
    await self.assertEqual(result, 'no_args_result')

  async def test_ellipsis_async(self):
    result = Chain(999).then(async_no_args_fn, ...).run()
    await self.assertEqual(result, 'async_no_args_result')
...
class TestThenArgumentPassing(MyTestCase):
  ...
  async def test_then_fn_with_kwargs_only(self):
    ...
    await self.assertEqual(result, {'x': 10})
...
class TestDoArgumentPassing(MyTestCase):
...
class TestExceptArgumentPassing(MyTestCase):
...
class TestFinallyArgumentPassing(MyTestCase):
...
class TestWithArgumentPassing(MyTestCase):
  ...
  async def test_with_fn_receives_ctx(self):
    ...
    await self.assertEqual(result, 'the_ctx')
    assert cm.entered
    assert cm.exited
  ...
  async def test_with_fn_with_args_ignores_ctx(self):
    ...
    await self.assertEqual(result, 'explicit_arg')
  ...
  async def test_with_fn_with_kwargs_ignores_ctx(self):
    ...
    await self.assertEqual(result, {'x': 42})
  ...
  async def test_with_async_cm(self):
    ...
    result = Chain(cm).with_(identity).run()
    await self.assertEqual(result, 'async_ctx')
    assert cm.entered
    assert cm.exited
  ...
  async def test_with_async_cm_with_args(self):
    ...
    await self.assertEqual(result, 'explicit')
...
# ===================================================================
# Section 7: foreach/filter/foreach_indexed argument passing
# ===================================================================

class TestForeachFilterArgumentPassing(MyTestCase):
  ...
  async def test_foreach_indexed_fn_receives_index_and_element(self):
    """foreach(fn, with_index=True) -> fn(index, element)."""
    result = Chain(['a', 'b', 'c']).foreach(lambda i, el: f'{i}:{el}', with_index=True).run()
  ...
  async def test_foreach_indexed_async(self):
    ...
    async def fmt(i, el):
      return f'{i}-{el}'
    result = Chain(['x', 'y']).foreach(fmt, with_index=True).run()
    await self.assertEqual(result, ['0-x', '1-y'])
...
class TestGatherArgumentPassing(MyTestCase):
  ...
  async def test_gather_each_fn_receives_current_value(self):
    ...
    await self.assertEqual(result, [11, 20, 7])
  ...
  async def test_gather_mixed_sync_async(self):
    ...
    await self.assertEqual(result, [20, 11])
...
class TestReturnBreakArgumentPassing(MyTestCase):
  ...
  async def test_break_with_callable_and_args_in_foreach(self):
    ...
    result = Chain([1, 2, 3, 4, 5]).foreach(maybe_break).run()
    await self.assertEqual(result, 300)
...
class TestRunArgumentPassing(MyTestCase):
...
# ===================================================================
# Section 11: Pipe operator argument passing
# ===================================================================

class TestPipeOperatorArgumentPassing(MyTestCase):
  """Test pipe operator (|) argument passing."""

  async def test_pipe_run_no_args(self):
    """chain | run() -> equivalent to chain.run()."""
    result = Chain(42) | run()
    await self.assertEqual(result, 42)

  async def test_pipe_run_with_override(self):
    """chain | run(value) -> equivalent to chain.run(value)."""
    result = Chain().then(identity) | run(42)
    await self.assertEqual(result, 42)

  async def test_pipe_run_with_callable_override(self):
    """chain | run(fn, args) -> chain.run(fn, args)."""
    result = Chain().then(identity) | run(add, 3, 7)
    await self.assertEqual(result, 10)

  async def test_pipe_append_value(self):
    """chain | value -> appends value as then(value, allow_literal=True)."""
    c = Chain(10) | (lambda v: v * 2)
    result = c.run()
    await self.assertEqual(result, 20)

  async def test_pipe_append_literal(self):
    """chain | literal -> appends literal."""
    c = Chain(10) | 42
    result = c.run()
    await self.assertEqual(result, 42)

  async def test_pipe_chain_sequence(self):
    """Multiple pipe operations."""
    result = Chain(1) | (lambda v: v + 1) | (lambda v: v * 3) | run()
    await self.assertEqual(result, 6)

  async def test_call_equivalent_to_run(self):
    """chain(value) -> equivalent to chain.run(value)."""
    result = Chain().then(identity)(42)
    await self.assertEqual(result, 42)

  async def test_call_with_args(self):
    """chain(fn, arg1) -> chain.run(fn, arg1)."""
    result = Chain().then(identity)(add, 3, 7)
    await self.assertEqual(result, 10)


# ===================================================================
# Section 12: Frozen chain argument passing
# ===================================================================

class TestFrozenChainArgumentPassing(MyTestCase):
  """Test frozen chain argument passing."""

  async def test_frozen_uses_original_root(self):
    """frozen().run() uses the original root."""
    frozen = Chain(42).then(identity).freeze()
    result = frozen.run()
    await self.assertEqual(result, 42)

  async def test_frozen_override_on_void_chain(self):
    """frozen.run(override) on void chain."""
    frozen = Chain().then(lambda v: v * 2).freeze()
    result = frozen.run(10)
    await self.assertEqual(result, 20)

  async def test_frozen_override_with_args(self):
    """frozen.run(fn, arg1) passes args to _make_temp_link."""
    frozen = Chain().then(identity).freeze()
    result = frozen.run(add, 3, 7)
    await self.assertEqual(result, 10)

  async def test_frozen_call_equivalent_to_run(self):
    """frozen(value) -> frozen.run(value)."""
    frozen = Chain().then(identity).freeze()
    result = frozen(42)
    await self.assertEqual(result, 42)

  async def test_frozen_reusable(self):
    """Frozen chain can be run multiple times."""
    frozen = Chain().then(lambda v: v * 2).freeze()
    await self.assertEqual(frozen.run(5), 10)
    await self.assertEqual(frozen.run(10), 20)
    await self.assertEqual(frozen.run(15), 30)
...
class TestDecoratorArgumentPassing(MyTestCase):
  ...
  async def test_decorator_basic(self):
    ...
    result = greet('world')
    await self.assertEqual(result, 'hello world decorated')
...
# ===================================================================
# Section 14: Cascade argument passing
# ===================================================================

class TestCascadeArgumentPassing(MyTestCase):
  """Test that Cascade passes root_value to every link, not previous result."""

  async def test_cascade_each_link_receives_root(self):
    """In Cascade, every then() receives the root value."""
    results = []

    def track(v):
      results.append(v)
      return v * 100  # This return value is ignored by next link

    result = Cascade(10).then(track).then(track).then(track).run()
    await self.assertEqual(result, 10)
    assert results == [10, 10, 10]

  async def test_cascade_with_args(self):
    """Cascade then(fn, arg1) still uses explicit args."""
    tracker = Tracker()
    result = Cascade(10).then(tracker, 'explicit').run()
    await self.assertEqual(result, 10)
    assert tracker.calls == [(('explicit',), {})]

  async def test_cascade_do_receives_root(self):
    """Cascade do() also receives root value."""
    results = []
...
class TestEdgeCases(MyTestCase):
  ...
  async def test_then_fn_many_kwargs(self):
    ...
    await self.assertEqual(result, {'a': 1, 'b': 2, 'c': 3, 'd': 4, 'e': 5})
  ...
  async def test_chain_of_chains(self):
    ...
    result = Chain(10).then(inner).run()
    await self.assertEqual(result, 110)
  ...
  async def test_frozen_override_kwargs(self):
    """frozen.run(fn, key=val)."""
    frozen = Chain().then(identity).freeze()
    result = frozen.run(kw_fn, x=42)
    await self.assertEqual(result, {'x': 42})
...
class TestComplexArgumentFlow(MyTestCase):
  ...
  async def test_explicit_args_override_in_pipeline(self):
    ...
    await self.assertEqual(pipeline, 55)
  ...
  async def test_nested_chain_arg_isolation(self):
    ...
    await self.assertEqual(result, 111)
  ...
  async def test_nested_chain_with_explicit_override(self):
    ...
    await self.assertEqual(result, 151)
...
class TestAsyncArgumentPassing(MyTestCase):
  ...
  async def test_async_break_in_foreach(self):
    ...
    await self.assertEqual(result, 'stopped_async')
...
class TestKwargsOnlyEdgeCases(MyTestCase):
...
class TestMakeTempLink(MyTestCase):
...
class TestCrossCuttingConcerns(MyTestCase):
  """Test interactions between different argument passing mechanisms."""

  async def test_cascade_with_explicit_args_in_do(self):
    """Cascade do(fn, arg) -> fn(arg), result discarded, chain returns root."""
    tracker = Tracker()
    result = Cascade(42).do(tracker, 'side_arg').run()
    await self.assertEqual(result, 42)
    assert tracker.calls == [(('side_arg',), {})]

  async def test_cascade_except_receives_root(self):
    """Cascade except_ handler receives root_value.
    Tracker returns root_value=42."""
    tracker = Tracker()
  ...
  async def test_frozen_chain_is_reusable_isolation(self):
    """Multiple runs of frozen chain are isolated."""
    frozen = Chain().then(lambda v: v * 2).freeze()
    r1 = frozen.run(5)
    r2 = frozen.run(10)
    r3 = frozen.run(15)
  ...
  async def test_chain_with_all_eval_codes_in_sequence(self):
    ...
    await self.assertEqual(result, 'final_literal')
  ...
  async def test_pipe_run_with_kwargs(self):
    ...
    await self.assertEqual(result, {'x': 99})
