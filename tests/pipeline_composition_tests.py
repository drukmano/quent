...
# ===========================================================================
# Class: TestPipelineWithFrozenChains
# ===========================================================================
class TestPipelineWithFrozenChains(unittest.TestCase):
  """Frozen chains as steps in larger pipelines."""

  def test_frozen_as_then_step(self):
    frozen = Chain().then(lambda x: x * 2).freeze()
    result = Chain(5).then(frozen).then(lambda x: x + 1).run()
    self.assertEqual(result, 11)

  def test_frozen_reusable(self):
    frozen = Chain().then(lambda x: x * 2).freeze()
    r1 = Chain(5).then(frozen).run()
    r2 = Chain(10).then(frozen).run()
    self.assertEqual(r1, 10)
    self.assertEqual(r2, 20)

  def test_frozen_in_complex_pipeline(self):
    frozen_double = Chain().then(_double).freeze()
    frozen_add1 = Chain().then(_add1).freeze()
    result = (
      Chain(5)
      .then(frozen_double)
      .then(frozen_add1)
  ...
  def test_frozen_called_directly(self):
    frozen = Chain().then(_double).then(_add1).freeze()
    result = frozen(5)
    self.assertEqual(result, 11)

  def test_frozen_does_not_modify_source_chain(self):
    """Freezing should not affect subsequent calls."""
    chain = Chain().then(_double)
    frozen = chain.freeze()
