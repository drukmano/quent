        ...
        try:
          await await_(
            ...
            .ignore(
              Cascade()
              .then(fn)
              .then(Chain().if_().else_(throw))
              .then(Chain().if_(lambda: False).else_(throw))
              .then(Chain().if_(lambda: False, ...).else_(throw))
              .then(Chain().not_().not_().else_(throw))
              .then(Chain().eq(2).else_(throw))
              .then(Chain().neq(1).else_(throw))
              .then(Chain().eq(1, throw))
              .then(Chain().then(type).is_(int).else_(throw))
              .then(Chain().then(type).is_not(str).else_(throw))
              .then(Chain().then(type).is_(str, throw))
              .then(Chain().in_([1, 2]).else_(throw))
              .then(Chain().not_in([1, 3]).else_(throw))
              .then(Chain().in_([1, 3], throw))
            )
            .ignore(
              Cascade()
            ...
            .ignore(Chain().neq(2).else_(lambda: 1))
            .ignore(Chain().neq(2).else_(lambda: 1, ...))
            .eq(2).else_(throw)
